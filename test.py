...
def test_compvis(model, prompt, sampler, steps=30,
 output_dir=None,
 seed=None,
 init_img=None, denoising_strength=0.75,
 sigma_override=None,
 filter_nsfw=False, safety_checker=None
):
    ...
    compvis.generate(prompt, sampler_name=sampler, ddim_steps=steps, seed=seed, init_img=init_img, sigma_override=sigma_override, denoising_strength=denoising_strength)
            ...
            for sampler in samplers:
                ...
                for steps in step_counts:
                    for sigma_override in sigma_overrides:
                        test_compvis(model, prompt, sampler, steps=steps, sigma_override=sigma_override, output_dir=output_dir)

            prompt = 'boobs'
            logger.info(f"Prompt: {prompt}")
            test_compvis(model, prompt, "k_lms", filter_nsfw=True, safety_checker=mm.loaded_models["safety_checker"]["model"], output_dir=output_dir)

            prompt = 'cute anime girl'
            logger.info(f"Prompt: {prompt}")
            for denoising_strength in denoising_strengths:
                test_compvis(model, prompt, "k_lms", init_img=init_image, denoising_strength=denoising_strength, output_dir=output_dir)
